    ...
    char *finals)
{
    ...
    /* input symbols: '0', '1', ... */
    cout << ("     | ");
    for (i = 0; i < nsymbols; i++) cout << ("  %c  ", '0'+i);
    ...
    for (i = 0; i < nstates; i++) {
        cout <<  'A' + i;  ("  %c  | ", 'A'+i);  /* state */
        for (j = 0; j < nsymbols; j++)
            cout << tab[i][j];    /* next state */
        cout << "\n";
    }
    cout << "Final states" << endl << finals;
}
...
int state_index(char *state, char stnt[][STATES+1], int n, int *pn,
    int cur)    /* 'cur' is added only for 'cout << ()' */
{
    int i;
    ...
    cout << cur << stnt[cur] << '\t' << state << state_flags << endl;
