...
struct Edge
{
	...
	// compare edges weights  no source/destination vertices
...
auto create_reference_graph()
{
    ...
    //     //     infile >> nodoA >> nodoB >> peso;
	// 	// 	G.add_edge(Edge<T>{&nodoA, &nodoB, &peso});
    //     // }
	...
	// 			infile >> nodoA >> nodoB >> peso;
	// 			// edges[i] = {nodoA, peso};
	// 			G.add_edge(Edge<T>{ i.nodoA, j.nodoB, j.peso });   
...
struct Label
{
	...
	// To compare labels their distances from source
...
auto prim_MST(const Graph<T>& G, size_t src)
{
	...
	// Search for the destination vertex
	while (!heap.empty())
	{
		auto current_vertex = heap.top();
		heap.pop();

		// vertice hasn't been visited in the past
